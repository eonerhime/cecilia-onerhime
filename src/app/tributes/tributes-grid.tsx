      ...
      <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {tributesState.map((tribute) => (
          <button
            ...
            className="group relative flex aspect-square flex-col justify-between overflow-hidden bg-[#536b60] p-8 text-left text-[#fbf8f2] transition-transform hover:-translate-y-1"
          >
            {reorderable && (
      ...
      {active && (
        ...
        >
          ...
          >
            {reorderable ? (
              <>
                <label className="block text-xs font-semibold uppercase tracking-[.2em] text-[#e4bb72]">
                  Name
                  <input
                    value={draftName}
                    onChange={(event) => setDraftName(event.target.value)}
                    maxLength={80}
                    className="mt-2 w-full border-b border-[#fbf8f2]/40 bg-transparent px-0 py-2 text-sm font-normal normal-case tracking-normal text-[#fbf8f2] outline-none"
                  />
                </label>
                <label className="mt-6 block text-xs font-semibold uppercase tracking-[.2em] text-[#e4bb72]">
                  Message
                  <textarea
                    value={draftMessage}
                    ...
                    maxLength={2000}
                    rows={8}
                    className="mt-2 w-full resize-none border-b border-[#fbf8f2]/40 bg-transparent px-0 py-2 text-base font-normal normal-case tracking-normal text-[#fbf8f2] outline-none"
                  />
                </label>
                <div className="mt-6 flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={close}
                    className="rounded-full border border-[#fbf8f2]/40 px-4 py-2 text-xs font-semibold text-[#fbf8f2]"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="rounded-full bg-[#c48a3a] px-4 py-2 text-xs font-semibold text-[#1f2d2b] disabled:opacity-60"
                  >
                    {saving ? "Saving..." : "Save"}
                  </button>
                </div>
